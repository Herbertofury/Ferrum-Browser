...
test('runtime spec resolves variables while evidence retains unresolved source placeholders', async () => {
...
test('EvidenceWriter sanitizes records, JSON, text, and returned final results', async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'ferrum-redact-evidence-'));
  try {
    const evidence = await new EvidenceWriter({ root, name: 'secret-safe' }).init();
    ...
    await evidence.writeJson('payload.json', { apiKey: 'json-secret', ok: true });
    await evidence.writeText('failure.txt', 'failed at https://user:pass@example.test/wd/hub?token=text-secret with Bearer bearer-secret');
    const result = await evidence.finalize({
      status: 'passed',
      result: { engine: 'appium', credentials: { password: 'result-secret' }, platformName: 'Android' }
    });
    ...
    for (const secret of ['user:pass', 'session-secret', 'capability-secret', 'json-secret', 'text-secret', 'bearer-secret', 'result-secret']) {
